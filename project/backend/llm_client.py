...
llm_client.py
─────────────
Cohere API wrapper with both streaming and non-streaming support.
Built using LangChain's ChatCohere interface.
"""
...
class CohereClient:
    def __init__(self) -> None:
        logger.info(f"Initialising Cohere client | model={settings.cohere_model}")

        self._model = ChatCohere(
            cohere_api_key=settings.cohere_api_key,
            model=settings.cohere_model,       # e.g. "command-a-03-2025"
        )
    ...
    def _build_history(self, history: List[dict]) -> list:
        """
        Converts internal history dicts to LangChain message objects.
    ...
    def generate(
        ...
        messages.append(HumanMessage(content=user_message))   # append current turn

        try:
            response = self._model.invoke(messages)
            answer = response.content
            logger.info(f"Cohere generated {len(answer)} chars for: '{query[:60]}'")
            return answer
        except Exception as exc:
        ...
        try:
            ...
            for chunk in self._model.stream(messages):
                ...
                if text:
                    ...
                    yield text
            logger.info(
                f"Cohere streamed {len(full_text)} chars for: '{query[:60]}'"
            )
        except Exception as exc:
            logger.error(f"Cohere stream error: {exc}")
            yield f"\n\n⚠️ Error generating response: {exc}"